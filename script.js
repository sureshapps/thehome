...
/* ================= GLOW CARD EDGE EFFECT =================
   Pointer-reactive glow for .glow-card sections (home-header, search-header,
   profile-header, profile-card, tabs, bottom-nav). Ported from the reference
   glow card: the glow's angle + intensity follow the pointer, fade out when
   not hovered, and each card gets a one-time intro sweep animation on load. */
document.addEventListener('DOMContentLoaded', () => {
    ...
    const pointerPositionRelativeToElement = (el, clientX, clientY) => {
        const { left, top, width, height } = el.getBoundingClientRect();
        const x = clientX - left;
        const y = clientY - top;
        return {
            pixels: [x, y],
            percent: [clamp((100 / width) * x), clamp((100 / height) * y)]
        };
    };
    ...
    function animateNumber({ startValue = 0, endValue = 100, duration = 1000, delay = 0, ease = (t) => t, onUpdate = () => {}, onEnd = () => {} }) {
        const startTime = performance.now() + delay;
        function tick() {
            const elapsed = performance.now() - startTime;
            const t = Math.min(Math.max(elapsed / duration, 0), 1);
            onUpdate(startValue + (endValue - startValue) * ease(t));
            if (elapsed < 0 || t < 1) {
    ...
    function playIntroSweep(card, delay) {
        ...
        animateNumber({
            ease: easeOutCubic, duration: 500, delay,
            onUpdate: (v) => card.style.setProperty('--pointer-d', v)
        });
        animateNumber({
            ease: easeInCubic, duration: 900, delay, endValue: 50,
            onUpdate: (v) => card.style.setProperty('--pointer-angle', `${(angleEnd - angleStart) * (v / 100) + angleStart}deg`)
        });
        animateNumber({
            ease: easeOutCubic, duration: 1300, delay: delay + 900, startValue: 50, endValue: 100,
            onUpdate: (v) => card.style.setProperty('--pointer-angle', `${(angleEnd - angleStart) * (v / 100) + angleStart}deg`)
        });
        animateNumber({
            ease: easeInCubic, duration: 900, delay: delay + 2200, startValue: 100, endValue: 0,
            onUpdate: (v) => card.style.setProperty('--pointer-d', v),
    ...
    glowCards.forEach((card, i) => {
        card.addEventListener('pointermove', (e) => {
            const { pixels } = pointerPositionRelativeToElement(card, e.clientX, e.clientY);
            const [px, py] = pixels;
        ...
        // Stagger the intro sweep slightly so all six don't glow in perfect unison
        setTimeout(() => playIntroSweep(card, 0), 600 + i * 180);
    });
});